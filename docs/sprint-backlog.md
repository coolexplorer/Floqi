...
> 스프린트 외 UX 개선 사항. 우선순위에 따라 스프린트에 편입.

| ID | Task | 상태 | Story Points | 비고 |
|----|------|------|-------------|------|
| UX-001 | 버튼 클릭/탭 시 시각적 피드백 애니메이션 추가 (ripple, scale, press 등) | ⬜ | 2 | 모든 Button 컴포넌트에 active/pressed 상태 애니메이션 적용. 현재 클릭 시 동작 인지 어려움 |
